...
gcloud pubsub subscriptions create my-sub --topic=my-topic \
    --push-endpoint=https://demo-agent-py.run.app/invoke
# Eventarc trigger
gcloud eventarc triggers create my-trigger \
    --destination-run-service=demo-agent-py \
...
*   **Correct (COMPLIANT):**
    ...
    root_agent = Agent(
        ...
        model="gemini-2.5-flash",  # PRESERVED
...
*   **Model Selection - CRITICAL:**
    *   **NEVER change the model unless explicitly asked.** If the code uses `gemini-2.5-flash`, keep it as `gemini-2.5-flash`. Do NOT "upgrade" or "fix" model names.
    *   When creating NEW agents (not modifying existing), use Gemini 2.5 series: `gemini-2.5-flash`, `gemini-2.5-pro`.
    *   Do NOT use older models (`gemini-1.5-flash`, etc.) unless the user explicitly requests them.
...
*   **Location Matters More Than Model:**
    ...
    *   Some models (like `gemini-2.5-flash`) require specific locations. Check the error message for hints.
